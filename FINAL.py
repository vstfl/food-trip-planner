...
def restaurant_information(text_query: str, inputs: dict, location: list, constraints: list):
    ...
    res = post_restaurant_information(body, constraints)
    places = res.get('places')
    if not places:
        print(res)
        raise Exception("No places?")
    return places
...
def main(inputs):
    ...
    # Constraint Module is too inconsistent, will disable for now
    ...
    all_places = []
    for q in query_list:
        res = restaurant_information(q + 'food', inputs, location, constraints)
    ...
    for day in range(days):
        ...
        for meal in inputs['meals']:
            ...
            loc = [float(place["location"]["latitude"]),
                   float(place["location"]["longitude"])]
            place_popup = generate_popup(name, loc)
            generate_marker(loc, 'red', 'cutlery', place_popup).add_to(m)
